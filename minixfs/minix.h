...
struct minix_sb_info {
	unsigned long s_ninodes; // inode总数
	unsigned long s_nzones; // 磁盘上全部的逻辑块的个数
	unsigned long s_imap_blocks; // inode表 位图块数 i节点位图区所使用的逻辑块的个数
	unsigned long s_zmap_blocks; // 数据块 位图块数 逻辑块位图区所使用的逻辑块的个数
	unsigned long s_firstdatazone; // 数据块的起点
	unsigned long s_log_zone_size; // 数据块长
	unsigned long s_max_size; // 最大文件字节长度
	int s_dirsize;
	int s_namelen; // 文件名长度
